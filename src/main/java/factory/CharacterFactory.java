    ...
    public static Character getCharacter(int type, int x, int y, int hp) {
        try {
            if(type == TEST_CHARACTER) {
                ...
                Texture[][] textures_char_moveAnimation = new Texture[4][4];
                Texture texture_tmp;
                texture_tmp = new Texture("./pics/char1/char1_down.png", 300, 600, 0, 0);
                for(int i = 0; i <= 3; i++) textures_char_moveAnimation[0][i] = texture_tmp.getCutTexture(0, i * 120, 150, 120, 100, 60);
                texture_tmp = new Texture("./pics/char1/char1_up.png", 300, 600, 0, 0);
                for(int i = 0; i <= 3; i++) textures_char_moveAnimation[1][i] = texture_tmp.getCutTexture(0, i * 120, 150, 120, 100, 60);
                texture_tmp = new Texture("./pics/char1/char1_left.png", 300, 600, 0, 0);
                for(int i = 0; i <= 3; i++) textures_char_moveAnimation[2][i] = texture_tmp.getCutTexture(0, i * 120, 150, 120, 100, 60);
                texture_tmp = new Texture("./pics/char1/char1_right.png", 300, 600, 0, 0);
                for(int i = 0; i <= 3; i++) textures_char_moveAnimation[3][i] = texture_tmp.getCutTexture(0, i * 120, 150, 120, 100, 60);
